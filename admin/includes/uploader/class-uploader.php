...
class WPEC_Email_Inquiry_Uploader extends WPEC_Email_Inquiry_Admin_UI
{
	...
	public function upload_input ( $name_attribute, $id_attribute = '', $value = '', $default_value = '', $field_name = '', $class = '', $css = '', $description = '', $post_id = 0 ) {
		...
		$output .= '<input id="upload_'.$id_attribute.'" class="a3rev-ui-upload-button a3_upload_button button" type="button" value="'.__( 'Upload', 'wp-e-commerce-catalog-visibility-and-email-inquiry' ).'" rel="'.$post_id.'" /> '.$description;
		...
		if ( $value != '' ) {
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'wp-e-commerce-catalog-visibility-and-email-inquiry' );
	...
	public function change_button_text( $translation, $original, $domain ) {
	    ...
	    if ( is_admin() && $original === 'Insert into Post' ) {
	    	$translation = __( 'Use this Image', 'wp-e-commerce-catalog-visibility-and-email-inquiry' );
			if ( isset( $_REQUEST['title'] ) && $_REQUEST['title'] != '' ) { $translation =__( 'Use as', 'wp-e-commerce-catalog-visibility-and-email-inquiry' ).' '.esc_attr( $_REQUEST['title'] ); }
	    }
	...
	public function modify_tabs ( $tabs ) {
		if ( isset( $tabs['gallery'] ) ) { $tabs['gallery'] = str_replace( 'Gallery', __( 'Previously Uploaded', 'wp-e-commerce-catalog-visibility-and-email-inquiry' ), $tabs['gallery'] ); }
		return $tabs;
	}
